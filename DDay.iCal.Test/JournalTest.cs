...
namespace DDay.iCal.Test
{
    ...
    public class JournalTest
    {
        ...
        public void Journal2()
        {
            ...
            Assert.IsTrue(string.Equals(j.Organizer.SentBy.OriginalString, "mailto:jane_doe@host.com", StringComparison.InvariantCultureIgnoreCase),
                "Organizer should have had been SENT-BY 'mailto:jane_doe@host.com'; it was sent by '" + j.Organizer.SentBy + "'");
            Assert.IsTrue(
                string.Equals(j.Organizer.DirectoryEntry.OriginalString, "ldap://host.com:6666/o=3DDC%20Associates,c=3DUS??(cn=3DJohn%20Smith)",
                    StringComparison.InvariantCultureIgnoreCase),
                "Organizer's directory entry should have been 'ldap://host.com:6666/o=3DDC%20Associates,c=3DUS??(cn=3DJohn%20Smith)', but it was '" +
                j.Organizer.DirectoryEntry + "'");
            Assert.AreEqual("MAILTO:jsmith@host.com", j.Organizer.Value.OriginalString);
            Assert.AreEqual("jsmith", j.Organizer.Value.UserInfo);
            Assert.AreEqual("host.com", j.Organizer.Value.Host);
            Assert.IsNull(j.Start);
        }
    }
}