...
namespace DDay.iCal
{
    ...
    public class Program
    {
        ...
        {   
            //iCalendar iCal = iCalendar.LoadFromFile(@"Calendars/cabin.ics");
            iCalendar iCal = iCalendar.LoadFromUri(new Uri("http://www.applegatehomecare.com/Calendars/cabin.ics"));
        ...
        public void RRULE1()
        {
            ...
            while (dt.Year < 2011)
            {
                ...
                else if (dt.Date != evt.DTStart.Date)
        ...
        public void RRULE3()
        {
            ...
            while (dt.Year < 1998)
            {
                    ...
                    (dt < new Date_Time(1997, 12, 24, 0, 0, 0, tzid, iCal)))
                {
                    Assert.IsTrue(evt.OccursAt(dt), "Event should occur at " + dt);
                    Assert.IsTrue(
                        (dt < new Date_Time(1997, 10, 26, tzid, iCal) && dt.TimeZoneInfo.TZName[0] == "EDT") ||
                        (dt > new Date_Time(1997, 10, 26, tzid, iCal) && dt.TimeZoneInfo.TZName[0] == "EST"),
        ...
        public void RRULE4()
        {
            ...
            for (int i = 0; i < DateTimes.Length; i++)
            {
                ...
                Assert.IsTrue(evt.OccursAt(dt), "Event should occur on " + dt);
                Assert.IsTrue(dt.TimeZoneInfo.TZName[0] == TimeZones[i], "Event " + dt + " should occur in the " + TimeZones[i] + " timezone");
            }
        ...
        public void RRULE23()
        {
            ...
            {
                Assert.IsTrue(evt.OccursAt(dt), "Event should occur on " + dt);
                Assert.IsTrue(dt.TimeZoneInfo.TZName[0] == "EDT", "Event " + dt + " should occur in the EDT TimeZone");
            }
        ...
        public void RRULE24()
        {
            ...
            {
                Assert.IsTrue(evt.OccursAt(dt), "Event should occur on " + dt);
                Assert.IsTrue(dt.TimeZoneInfo.TZName[0] == "EST", "Event " + dt + " should occur in the EST TimeZone");   
            }
